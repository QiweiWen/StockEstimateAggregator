...
for l in lines:
    ...
    outstanding = int(resobj.group (5))
    mktval = price * outstanding
    spret = float(resobj.group (6))
    dateobj = re.search (r"^([0-9]{4})([0-9]{2})", date)
    year = int(dateobj.group (1))
    month = int(dateobj.group (2))
    print "insert into monthlystock (\"cusip\", \"year\", \"month\",\"mktval\",\"spret\") values (\'%s\', %d, %d, %lf, %lf);" % (cusip, year, month, mktval, spret)
